...
# In useage like:
#
#    ansible-vault --vault-id keying_id@contrib/vault/vault-keyring-client.py view some_encrypted_file
#
...
def main():
    ...
    username = (
        get_ini_config_value(config._parsers, dict(section="vault", key="username"))
        or getpass.getuser()
    )
    keyname = (
        get_ini_config_value(config._parsers, dict(section="vault", key="keyname"))
        or "ansible"
    )
